...
class Fixings:
    ...
    @staticmethod
    def _load_csv(directory: str, path: str) -> pd.Series[float]:
        target = os.path.join(directory, path)
        if version.parse(pd.__version__) < version.parse("2.0"):  # pragma: no cover
            ...
            df = pd.read_csv(target, index_col=0, parse_dates=[0], date_format="%d-%m-%Y")
        return df["rate"].sort_index(ascending=True)

    def __getitem__(self, item: str) -> pd.Series[float]:
    ...
    def __init__(self) -> None:
        ...
        self.loaded: dict[str, pd.Series[float]] = {}
